...
def ConsistentWith (cov : CovMap) (R : RoleAssign Instance n) : Prop :=
  ∀ r, CertificationLogic.coverage (R r) = cov r

/-- Canonical role assignment from a coverage map. -/
def canonicalRoleAssign (cov : CovMap) : RoleAssign Instance n :=
  fun r => CertificationLogic.canonicalVerifier (cov r)

theorem ConsistentWith_canonical (cov : CovMap) :
...
theorem protocolCoverage_eq_of_ConsistentWith (cov : CovMap)
    (R : RoleAssign Instance n)
    (hR : ConsistentWith cov R) (P : Prot n) :
...
def CertifiableAt {Stratum : Type*} (cov : CovMap) (_S : Stratum) (C : Formula) : Prop :=
  ∃ (P : Prot n) (R : RoleAssign Instance n),
