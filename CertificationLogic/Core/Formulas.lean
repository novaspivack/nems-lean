...
/-- **ProtocolDerivable** (⊢_S C): syntactic proof system mirroring protocol combinators.
  Stratum S is abstract; for single-stratum use Unit. -/
inductive ProtocolDerivable {Stratum : Type*} (cov : Role n → Finset Instance)
    (ax : Stratum → Finset Instance → Prop) : Stratum → Finset Instance → Prop
  | ax (S : Stratum) (C : Finset Instance) (h : ax S C) : ProtocolDerivable cov ax S C
  | union (S : Stratum) (C₁ C₂ : Finset Instance) (h1 : ProtocolDerivable cov ax S C₁)
      (h2 : ProtocolDerivable cov ax S C₂) : ProtocolDerivable cov ax S (C₁ ∪ C₂)
  | subset (S : Stratum) (C C' : Finset Instance) (h : ProtocolDerivable cov ax S C)
      (hsub : C' ⊆ C) : ProtocolDerivable cov ax S C'
  | stratumMono (S S' : Stratum) (C : Finset Instance) (_h : ProtocolDerivable cov ax S C)
      (hle : S = S') : ProtocolDerivable cov ax S' C  -- intended: S ≤ S' (strength preorder, Paper 29)
