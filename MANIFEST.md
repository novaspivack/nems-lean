...
| `SemanticSelfDescription/Theorems/KleenePredicatedSummit.lean` | `kleene_computational_predicated_summit` | **Predicated summit theorem** (nonempty **`BarrierHypothesesPred`** for Kleene) |
| `StructuralNonExhaustibility/Bridges/ToSemanticSelfDescription.lean` | `reflexiveSystem_ofSelfSemanticFramePred`, `reflexiveSystem_ofSelfSemanticFramePred_noTotalExhaustion_of_forall` | Program V shell with **`Nonempty (BarrierHypothesesPred F P)`**; **`NoTotalExhaustion`** when **`∀ F', P F'`** |
